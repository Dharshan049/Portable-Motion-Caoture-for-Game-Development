import { compileJSONPointer, existsIn, getIn, immutableJSONPatch, setIn, updateIn } from 'immutable-json-patch';
import { initial, isEqual, last, uniqueId } from 'lodash-es';
import { DEFAULT_VISIBLE_SECTIONS, STATE_ENFORCE_STRING, STATE_EXPANDED, STATE_ID, STATE_KEYS, STATE_VISIBLE_SECTIONS } from '../constants.js';
...
export function expandPath(json, state, path) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        if (Array.isArray(getIn(updatedState, partialPath))) {
            ...
            const sections = (getIn(updatedState, sectionsPath) ||
                DEFAULT_VISIBLE_SECTIONS);

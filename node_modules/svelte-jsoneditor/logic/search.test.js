...
import { createRecursiveSearchResults, createSearchAndReplaceAllOperations, createSearchAndReplaceOperations, findCaseInsensitiveMatches, replaceText, search, splitValue } from './search.js';
describe('search', () => {
    it('search in JSON', () => {
        const json = {
            b: { c: 'a' },
    ...
    it('should limit search results to the provided max in case of multiple matches in a single field', () => {
        const maxResults = 4;
        assert.deepStrictEqual(search('ha', { greeting: 'ha ha ha ha ha ha' }, undefined, maxResults).length, maxResults);
        assert.deepStrictEqual(search('ha', { 'ha ha ha ha ha ha': 'ha ha ha ha ha ha' }, undefined, maxResults).length, maxResults);
    });
    ...
    it('should create operations to replace a search result value', () => {
        ...
        const { operations, newSelection } = createSearchAndReplaceOperations(json, state, '*', results[2]);
    ...
    it('should create operations to replace a search result key', () => {
        ...
        const state = syncState(json, undefined, [], () => true);
        const results = search('world', json, state);
        const { operations, newSelection } = createSearchAndReplaceOperations(json, state, '*', results[0]);
        assert.deepStrictEqual(operations, [
    ...
    it('should create operations to replace all search results', () => {
        ...
        const { operations, newSelection } = createSearchAndReplaceAllOperations(json, state, searchText, replacementText);
        assert.deepStrictEqual(operations, [
            {
                op: 'replace',
    ...
    it('should create operations to replace all search results matching a numeric value', () => {
        ...
        const searchText = '2';
        const replacementText = '*';
        const { operations } = createSearchAndReplaceAllOperations(json, state, searchText, replacementText);
        assert.deepStrictEqual(operations, [
            {
                op: 'replace',
                path: '/value',
